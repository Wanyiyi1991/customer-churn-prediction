...
st.set_page_config(page_title="客户流失预测系统", page_icon="📊", layout="wide")
...
def load_artifacts():
    ...
    st.write(f"🔍 预处理器: {preprocessor_path} | 存在: {os.path.exists(preprocessor_path)}")
    with gzip.open(preprocessor_path, 'rb') as f:
        preprocessor = joblib.load(f)
    
    model_path = os.path.join(current_dir, 'churn_model.json')
    st.write(f"🔍 模型: {model_path} | 存在: {os.path.exists(model_path)}")
    model = xgb.XGBClassifier()
    model.load_model(model_path)
    
    feature_path = os.path.join(current_dir, 'feature_names.pkl')
    st.write(f"🔍 特征名: {feature_path} | 存在: {os.path.exists(feature_path)}")
    feature_names = joblib.load(feature_path)
    
    st.write(f"📋 预处理器期望列: {list(preprocessor.feature_names_in_)}")
...
if predict_btn:
    ...
    st.write("输入数据:", input_data)
    st.write("输入类型:", input_data.dtypes)
    
    try:
        input_processed = preprocessor.transform(input_data)
        st.success(f"转换成功！输出形状: {input_processed.shape}")
...
st.caption("数据来源: Telco Customer Churn (Kaggle)")
    ...
    st.info("👈 请在左侧输入客户信息，然后点击「预测流失概率」按钮") Telco Customer Churn (Kaggle)")